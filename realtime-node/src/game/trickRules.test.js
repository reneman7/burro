...
test('sin el palo pedido pero con triunfo, NO es obligatorio triunfar: libertad total', () => {
  ...
  assert.equal(isAchico, false, 'todavía no se jugó triunfo en la baza, así que no aplica "me achico"');
});
...
test('post-triunfo: con el palo pedido, sigue palo obligatorio aunque tenga un triunfo mayor', () => {
  ...
  // Tiene el palo pedido (el 2 de tréboles): solo ese es válido, aunque tenga triunfos.
  assert.deepEqual(validIndexes, [0]);
  assert.equal(isAchico, false);
});

test('post-triunfo: sin el palo pedido, libertad total aunque tenga un triunfo que no supera', () => {
  const hand = [c('K', TRUMP), c('4', TRUMP), c('9', 'diamantes')];
  const { validIndexes, isAchico } = getValidPlays(
    hand,
    { ledSuit: 'treboles', trumpPlayed: true, highestTrumpRank: 'A' },
    TRUMP
  );
  assert.deepEqual(validIndexes, [0, 1, 2]);
  assert.equal(isAchico, true, 'tiene triunfos (K, 4) pero ninguno supera al As ya jugado');
});

test('post-triunfo: sin palo pedido y con un triunfo que SÍ supera -> no es "me achico"', () => {
  const hand = [c('A', TRUMP), c('9', 'diamantes'), c('2', 'espadas')];
  const { validIndexes, isAchico } = getValidPlays(
    hand,
  ...
  assert.deepEqual(validIndexes, [0, 1, 2]);
  assert.equal(isAchico, false, 'tiene un triunfo (As) que sí supera al Rey ya jugado, así que no se achica');
});

test('teniendo el palo pedido, jugar otra cosa fuera del conjunto válido sería renuncia (no está en validIndexes)', () => {
